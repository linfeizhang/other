...
//    self.view.backgroundColor = [UIColor orangeColor];
    // 创建扫描边框
    self.scanningView = [[SGScanningQRCodeView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height) outsideViewLayer:self.view.layer];
...
- (void)scanQRCodeFromPhotosInTheAlbum:(UIImage *)image {
    ...
    for (int index = 0; index < [features count]; index ++) {
        ...
        if (self.first_push) {
            ...
            if ([self.delegate respondsToSelector:@selector(receiveScanningResult:)]) {
                [self.delegate receiveScanningResult:scannedResult];
            }
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects.count > 0) {
        ...
        [self.navigationController popViewControllerAnimated:YES];
        if ([self.delegate respondsToSelector:@selector(receiveScanningResult:)]) {
            [self.delegate receiveScanningResult:obj.stringValue];
        }
