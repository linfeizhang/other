...
-(CGRect)getViewFrame
{
    ...
    NSInteger countNum = [self.titleArray count];
    if (countNum <8) {
        frame.size.height = [self.titleArray count] * ROW_HEIGHT + SPACE + kArrowHeight;
    }else{
        frame.size.height = 8 * ROW_HEIGHT + SPACE + kArrowHeight;
    }
        
    for (NSString *title in self.titleArray) {
        CGFloat width = [title boundingRectWithSize:CGSizeMake(170, 80) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:TITLE_FONT} context:nil].size.width;
    ...
    if ([self.titleArray count] == [self.imageArray count]) {
        ...
        frame.size.width = 10 + frame.size.width + 25;
    }
    
    frame.origin.x = self.showPoint.x - frame.size.width/2;
    
    if (SCREENHEIGHT - self.showPoint.y < frame.size.height) {
        frame.origin.y = self.showPoint.y-frame.size.height-26;
    }else{
